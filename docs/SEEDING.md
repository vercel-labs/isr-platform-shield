...
The seed script reads `subdomains.primary` and `subdomains.secondary` from `validation.json`. If you rename them in config, run `pnpm seed` to create the new entries. Old Redis keys are not removed automatically.
